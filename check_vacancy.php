...
*/
	...
	$sql = "SELECT `tag_occupied_by_user`.`tag_id`,  `tag_to_table`.`table_id`, `tag_occupied_by_user`.`timestamp_ending`
FROM `tag_to_table` INNER JOIN `table_to_canteen` ON `tag_to_table`.`table_id` = `table_to_canteen`.`table_id`
LEFT JOIN `tag_occupied_by_user`
ON `tag_occupied_by_user`.`tag_id`=`tag_to_table`.`tag_id`
